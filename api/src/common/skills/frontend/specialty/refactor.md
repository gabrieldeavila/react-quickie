...
- **Explain The "What" and "Why"**: Before outputting _any_ code, you MUST write a short paragraph explaining exactly _what_ you are about to do and _why_ it is necessary according to the target architecture.
...
- **Smart Declaration Search**: When auditing for misplaced contracts, you must find where types are _declared_ (including local/non-exported ones), not imported. Search for patterns like `interface `, `type [Name] =`, or `enum `. You MUST actively exclude/ignore any results that are part of an `import` statement (e.g., `import type`, `import { ... }`). Do not flag files that merely consume types.
...
**⚠️ Strict Context Typing Rule:** NEVER define types or interfaces alongside the context files. The interfaces for both the `BaseContext` and the `ServicesContext` MUST be declared together in a single file inside the global `types/interface/` folder (e.g., `types/interface/[ComponentName]Context.interface.ts`).
...
_"Audit Brief: The `UserProfile.tsx` file is a God File. It mixes TypeScript interfaces, API calls, state management, and UI rendering. It violates the strict folder structure and needs to be decomposed into centralized types, a Context provider, and a feature content file."_
...
_Stop here and ask the user: "Shall we proceed with Step 1?"_
...
**1. The "What" and "Why"**
_(CRITICAL RULE: Maximum 2 short sentences. Do not repeat instructions. Just state the immediate action and the direct reason)._
_"**Action:** Extracting `User` interface to `types/interface/user.interface.ts`.
**Reason:** Centralizing contracts prevents duplicate declarations and keeps components focused on UI."_

**2. The Code**
_(CRITICAL RULE: ALWAYS output the full, complete, and runnable code for the current step. NEVER use `...` or omit logic. The user must be able to copy and paste your exact output. Do not skip lines just to save space)._
...
**Transparency Builds Trust.** Refactoring can break things. By explaining _why_ a piece of code is moving, the user learns the architecture and can catch potential edge cases you might miss.
